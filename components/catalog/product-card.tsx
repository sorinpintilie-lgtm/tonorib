...
import { fetchSellerById } from '@/lib/firebase-data';
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  useEffect(() => {
    fetchSellerById(product.sellerId).then(setSeller).catch(() => setSeller(null));
  }, [product.sellerId]);
  ...
  const getFreshnessStatus = () => {
    ...
    if (daysDiff < 0) return { label: 'Expired', className: 'badge-stock', Icon: AlertCircle };
    if (daysDiff <= 1) return { label: 'Fresh Today', className: 'badge-fresh', Icon: Clock };
    if (daysDiff <= 3) return { label: 'Fresh', className: 'badge-fresh', Icon: Clock };
    return null;
  };
    ...
    <div className="card card-hover overflow-hidden group">
      <Link href={`/product/${product.id}`} className="block relative aspect-[4/3] overflow-hidden bg-seafoam">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
          {freshnessStatus && (
            <span className={`badge ${freshnessStatus.className} flex items-center gap-1`}>
      ...
      <div className="p-4">
        ...
        <div className="flex items-end justify-between gap-2">
          <div>
            ...
            {product.minOrderKg > 1 && <p className="text-xs text-slate-400">Min. Order: {product.minOrderKg} kg</p>}
          </div>

          <Button
            variant="secondary"
            size="sm"
            onClick={() => addItem(product.id, product.minOrderKg)}
            disabled={!isInStock}
